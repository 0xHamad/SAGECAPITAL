...
const plans = [
  ['Starter', '$10', '5% – 8%', '0.1%', '#64748b'], ['Basic', '$20', '5% – 10%', '0.2%', '#3b82f6'], ['Explorer', '$30', '6% – 11%', '0.3%', '#7c3aed'], ['Bronze', '$50', '7% – 12%', '0.5%', '#d97706'], ['Silver', '$100', '8% – 13%', '1.0%', '#64748b'], ['Gold', '$200', '10% – 14%', '2.0%', '#ca8a04'], ['Platinum', '$500', '12% – 15%', '5.0%', '#0891b2'],
]
  ...
  return <div className={styles.shell}>
    ...
    <div className={styles.content}>
      ...
      <main className={styles.main}>
        ...
        {active === 'Profile' && <Profile styles={styles} />}
      </main>
...
function Plans({ styles }: { styles: ReturnType<typeof useStyles> }) { return <><section className={styles.hero}><div className={styles.globe} /><div className={styles.heroContent}><Badge appearance="tint" color="success">⚡ Simple, automated investing</Badge><div className={styles.heroTitle}>Choose a Plan — Auto Trading Starts Immediately</div><Body1 className={styles.heroText}>Put your capital to work with a plan designed around your goals. Weekly returns are credited automatically.</Body1></div></section><div className={styles.info}><strong>⚡ How it works: </strong>Your plan runs automated crypto trading on SageCapital's engine. Each week, the engine generates a random return between your plan's minimum and maximum percentage. Returns are credited every 7 days automatically.</div><div className={styles.planGrid}>{plans.map(([name, price, range, referral, color], i) => <div className={styles.planCard} style={{ borderTop: `3px solid ${color}` }} key={name}><div className={styles.metricTop}><Badge appearance="tint" style={{ color }}> {i === 4 ? '★ Most Popular' : i === 5 ? '🔥 Best Value' : i === 6 ? '🏆 Elite' : 'Tier ' + (i + 1)}</Badge><Add24Regular /></div><Subtitle1>{name}</Subtitle1><Title2>{price}</Title2><div className={styles.planAccent} style={{ color }}> {range}</div><Caption1 className={styles.muted}>Random weekly return via auto trading</Caption1><Divider /><Body2>Min scenario: <strong>{price === '$10' ? '$0.50' : price === '$100' ? '$8.00' : price === '$500' ? '$60.00' : 'variable'}/week</strong></Body2><Body2>⚡ Fully Automated — No check-in needed</Body2><Body2>👥 Refer & earn {referral} of profit</Body2><Button appearance="primary" icon={<ArrowRight24Regular />}>Buy {name} Plan</Button></div>)}</div></> }

function Wallet({ styles, copied, copyLink }: { styles: ReturnType<typeof useStyles>; copied: boolean; copyLink: () => void }) { return <><Title1>Wallet</Title1><Body1 className={styles.muted}>Manage deposits, withdrawals, and transaction history.</Body1><div className={styles.metrics}>{[['Total Balance','$1,567.32'],['Withdrawable Earnings','$134.50'],['Total Deposited','$1,400.00']].map(([a,b]) => <div className={styles.metric} key={a}><Caption1 className={styles.muted}>{a}</Caption1><div className={styles.metricValue}>{b}</div><Caption1 className={styles.green}>Available balance</Caption1></div>)}</div><Card className={styles.tableCard}><TabList defaultSelectedValue="deposit"><Tab value="deposit">💳 Deposit</Tab><Tab value="withdraw">📤 Withdraw</Tab></TabList><div style={{ paddingTop: '20px', display: 'flex', flexDirection: 'column', gap: '16px' }}><Subtitle2>Select a network</Subtitle2><div className={styles.planGrid}>{['💵 USDT BEP20','🔵 USDC BEP20','🟡 BNB BEP20'].map(coin => <Button key={coin} appearance="outline">{coin}</Button>)}</div><MessageBar intent="info"><MessageBarBody><MessageBarTitle>Monitoring blockchain</MessageBarTitle>SageCapital auto-confirms deposits in 1–15 minutes.</MessageBarBody></MessageBar><div className={styles.info}>Master wallet · <strong>0x71A4...9f2E</strong><Button appearance="subtle" icon={copied ? <CheckmarkCircle24Regular /> : <Copy24Regular />} onClick={copyLink}>{copied ? 'Copied' : 'Copy'}</Button></div></div></Card></> }

function Referrals({ styles, copied, copyLink }: { styles: ReturnType<typeof useStyles>; copied: boolean; copyLink: () => void }) { return <><section className={styles.hero}><div className={styles.globe} /><div className={styles.heroContent}><Badge appearance="tint" color="success">💎 Built for your network</Badge><div className={styles.heroTitle}>SageCapital Referral Network</div><Body1 className={styles.heroText}>Earn 3 levels deep when your referrals invest. Commission is paid automatically every Monday.</Body1></div></section><Card className={styles.tableCard}><Subtitle2>Your referral link</Subtitle2><div style={{ display: 'flex', gap: '10px', alignItems: 'center', flexWrap: 'wrap', paddingTop: '12px' }}><Input value="sagecapital.io/ref/USER123" readOnly style={{ flex: 1, minWidth: '220px' }} /><Button appearance="primary" icon={copied ? <CheckmarkCircle24Regular /> : <Copy24Regular />} onClick={copyLink}>{copied ? 'Copied' : 'Copy link'}</Button></div></Card><div className={styles.metrics}>{[['Total Network','8'],['Level 1','3'],['Level 2','4'],['Total Earned','$18.60']].map(([a,b]) => <div className={styles.metric} key={a}><Caption1 className={styles.muted}>{a}</Caption1><div className={styles.metricValue}>{b}</div></div>)}</div><section className={styles.tableCard}><Title3>Commission structure</Title3><table className={styles.table}><thead><tr>{['Level','Who','$10 Plan','$50 Plan','$100 Plan','$500 Plan'].map(h => <th className={styles.th} key={h}>{h}</th>)}</tr></thead><tbody>{[['🥇 Level 1','Direct referral','0.1%','0.5%','1.0%','5.0%'],['🥈 Level 2','Their referrals','0.01%','0.05%','0.1%','0.5%'],['🥉 Level 3','3rd generation','0.001%','0.005%','0.01%','0.05%']].map(row => <tr key={row[0]}>{row.map(x => <td className={styles.td} key={x}>{x}</td>)}</tr>)}</tbody></table></section></> }

function Profile({ styles }: { styles: ReturnType<typeof useStyles> }) { return <><div className={styles.user}><Avatar name="Jordan Lee" color="brand" size={64} /><div><Title1>Jordan Lee</Title1><Body1 className={styles.muted}>jordan.lee@example.com · SageCapital member since Aug 2026</Body1></div></div><div className={styles.planGrid}><Card className={styles.tableCard}><CardHeader header={<Subtitle2>Account Info</Subtitle2>} image={<Person24Regular />} /><Body2>Name, email, and verification status</Body2><Badge appearance="tint" color="success">✓ Identity verified</Badge></Card><Card className={styles.tableCard}><CardHeader header={<Subtitle2>Security</Subtitle2>} image={<LockClosed24Regular />} /><Body2>Manage your password and two-factor authentication.</Body2><Button appearance="outline">Manage security</Button></Card><Card className={styles.tableCard}><CardHeader header={<Subtitle2>Linked Wallet</Subtitle2>} image={<WalletCreditCard24Regular />} /><Body2>0x71A4...9f2E · BEP20</Body2><Button appearance="outline">Update wallet</Button></Card><Card className={styles.tableCard}><CardHeader header={<Subtitle2>Settings & Support</Subtitle2>} image={<Settings24Regular />} /><Body2>Notifications, FAQs, and legal documents</Body2><Button appearance="primary">Open support center</Button></Card></div></> }
