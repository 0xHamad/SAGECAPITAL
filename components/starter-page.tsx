...
function Wallet({ styles, copied, copyLink }: { styles: ReturnType<typeof useStyles>; copied: boolean; copyLink: () => void }) {
  ...
  const [coin, setCoin] = React.useState('USDT BEP20')
  return <><Title1>Wallet</Title1><Body1 className={styles.muted}>Manage deposits, withdrawals, and transaction history.</Body1><div className={styles.metrics}>{[['Total Balance','$1,567.32'],['Withdrawable Earnings','$134.50'],['Total Deposited','$1,400.00']].map(([a,b]) => <div className={styles.metric} key={a}><Caption1 className={styles.muted}>{a}</Caption1><div className={styles.metricValue}>{b}</div><Caption1 className={styles.green}>Available balance</Caption1></div>)}</div><Card className={styles.tableCard}><TabList selectedValue={tab} onTabSelect={(_, data) => setTab(String(data.value))}><Tab value="deposit">Deposit</Tab><Tab value="withdraw">Withdraw</Tab></TabList>{tab === 'deposit' ? <div style={{ paddingTop: '20px', display: 'flex', flexDirection: 'column', gap: '16px' }}><Subtitle2>Select a network</Subtitle2><div className={styles.planGrid}>{['USDT BEP20','USDC BEP20','BNB BEP20'].map(coin => <Button key={coin} appearance="outline">{coin}</Button>)}</div><MessageBar intent="info"><MessageBarBody><MessageBarTitle>Monitoring blockchain</MessageBarTitle>SageCapital auto-confirms deposits in 1–15 minutes.</MessageBarBody></MessageBar><div className={styles.info}>Master wallet · <strong>0x71A4...9f2E</strong><Button appearance="subtle" icon={copied ? <CheckmarkCircle24Regular /> : <Copy24Regular />} onClick={copyLink}>{copied ? 'Copied' : 'Copy'}</Button></div></div> : submitted ? <div style={{ paddingTop: '24px', display: 'flex', flexDirection: 'column', gap: '14px', alignItems: 'center', textAlign: 'center' }}><CheckmarkCircle24Regular style={{ color: '#159447', fontSize: '48px' }} /><Title3>Request Submitted!</Title3><Body1>Your withdrawal will be processed this Monday.</Body1></div> : <div style={{ paddingTop: '20px', display: 'flex', flexDirection: 'column', gap: '16px' }}><MessageBar intent="success"><MessageBarBody><MessageBarTitle>Withdrawable Balance: $134.50</MessageBarTitle>Only your earned weekly profits can be withdrawn. Your invested principal stays active in trading.</MessageBarBody></MessageBar><MessageBar intent="warning"><MessageBarBody>Withdrawals are processed every Monday. Requests submitted anytime during the week will be paid next Monday.</MessageBarBody></MessageBar><Subtitle2>Select Coin to Receive</Subtitle2><div className={styles.planGrid}>{['USDT BEP20','USDC BEP20','BNB BEP20'].map(value => <Button key={value} appearance={coin === value ? 'primary' : 'outline'} onClick={() => setCoin(value)}>{value}</Button>)}</div><label><Body2>Your Wallet Address (BEP20)</Body2><Input style={{ width: '100%', marginTop: '6px' }} placeholder="0x... enter your BEP20 receiving address" /></label><label><Body2>Amount to Withdraw (USD)</Body2><Input style={{ width: '100%', marginTop: '6px' }} placeholder="Enter amount (Available: $134.50)" /><Caption1 className={styles.muted}>Network fee: ~134.00</Caption1></label><Button appearance="primary" onClick={() => setSubmitted(true)}>Submit Withdrawal Request</Button></div>}</Card></> }
