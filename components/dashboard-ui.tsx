...
export function Wallet({ styles, copied, copyLink }: { styles: ReturnType<typeof useStyles>; copied: boolean; copyLink: () => void; }) {
  ...
  const handleCreateDeposit = async () => {
    ...
    if (isNaN(amt) || amt < 1) return setError('Minimum deposit is $1.00 USD')
    setLoading(true); setError('')
    try {
      const res = await fetch('/api/deposit/create', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ amount: amt, coin: 'USDTBSC' })
      })
      const data = await res.json()
